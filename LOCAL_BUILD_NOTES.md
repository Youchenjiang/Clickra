...
- **Cross-process Sync & Locking**:
  ...
  - Named Mutexes (`Global\Clickra_Language_Mutex`) and file locks are utilized to normalize language and prevent race conditions when reading/writing settings from the package folder in MSIX.
...
- **Git Workflow**: Direct commits to `main` are forbidden. Use `feature/...` or `hotfix/...` branches and submit Pull Requests. Tag creation and push rules must follow: local tags in `vX.Y.Z.0` format must be pushed directly via `git push origin vX.Y.Z.0`. Direct branch pushes to remote are prohibited.

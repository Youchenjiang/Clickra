...
namespace Clickra.UI
{
    ...
    public partial class ProgressWindow
    {
        ...
        private void Paint(IntPtr hdc)
        {
            ...
            float s = _dpiScale;
            PaintTitleAndSubtitle(g, s, hasErr, comp, isPrompting);

            if (hasErr)
            {
                PaintErrorState(g, s, errMsg);
            }
            else if (comp)
            {
                PaintSuccessState(g, s, msg);
            }
            else if (isPrompting)
            {
                PaintPromptState(g, s, promptFile, isRetry);
            }
            else
            {
                PaintProgressState(g, s, msg, cur, tot, dispW, shimOff);
            }
        ...
        /// <summary>Paints the failure state: red header and the localized error message.</summary>
        private void PaintErrorState(Graphics g, float s, string errMsg)
        {
            if (_headerFont != null)
            {
        ...
        /// <summary>Paints the success state: green header, completion message and the
        /// auto-close hint.</summary>
        private void PaintSuccessState(Graphics g, float s, string msg)
        {
            if (_headerFont != null)
            {
        ...
        /// <summary>Paints the visual splitter UI or the localized password prompt.</summary>
        private void PaintPromptState(Graphics g, float s, string promptFile, bool isRetry)
        {
        ...
        /// <summary>Paints the in-progress state: scrollable status message, animated
        /// progress bar, percentage, tip and the minimize-to-tray button.</summary>
        private void PaintProgressState(Graphics g, float s, string msg, int cur, int tot, double dispW, float shimOff)
        {
            PaintProgressMessage(g, s, msg, cur, tot);
            PaintProgressBar(g, s, dispW, shimOff);

            string pctStr = tot > 0 ? $"{(cur * 100 / tot)}%" : "";
            if (_pctFont != null)
            {
        ...
        /// <summary>Paints the status message, clipping and scrolling it when it overflows,
        /// and drawing the horizontal scrollbar when scrollable.</summary>
        private void PaintProgressMessage(Graphics g, float s, string msg, int cur, int tot)
        {
            if (_msgFont == null) return;
        ...
        /// <summary>Paints the progress-bar track and the animated gradient fill with its
        /// shimmer highlight.</summary>
        private void PaintProgressBar(Graphics g, float s, double dispW, float shimOff)
        {
        ...
        /// <summary>Paints the minimize-to-tray button with its hover state and tooltip.</summary>
        private void PaintTrayButton(Graphics g, float s)
        {
        ...
        /// <summary>Blits the back buffer to the device context, leaving the password input
        /// controls visible (their areas are excluded from the blit).</summary>
        private void PaintToHdc(IntPtr hdc, float s)
        {

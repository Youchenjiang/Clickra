...
### E. 表格數據與純數值 (Tables & Numeric Data)
*   **特徵**：經 `MarkTableParagraphs` 幾何分群判定為表格單元格，且單字量 $\le 10$。
*   **處理**：標記為 `IsTable = true`（注意：這與 `IsBypassed` 獨立）。這類段落如果是純數據（無英文字母），則會被 Bypass；如果是文字，則在 Pass 1 中會防止被白色背景遮罩覆蓋，重繪時不使用白色背景，以避免抹除表格的框線。

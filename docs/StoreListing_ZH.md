...
以下是供您直接複製貼上至 Microsoft 合作夥伴中心 (Partner Center) 的完整欄位資訊，已更新至 v3.2.0.0 的最新狀態。
...
## 說明 (Description)
【系統需求】「文書轉PDF」功能需於本機裝置安裝對應之 Microsoft Word 或 PowerPoint。

Clickra 是一款專為 Windows 10 與 Windows 11 打造的極速原生右鍵選單工具集。它能無縫整合至檔案總管選單，無須開啟任何龐大臃腫的軟體，就能瞬間完成檔案處理。

核心功能：
　　原生儀表板：全新深色模式介面，即時監測 Office 組件安裝狀態。
　　文書轉 PDF：支援 Word (.doc/.docx) 與 PPT (.ppt/.pptx) 靜默轉檔，高品質不跑版。
...
- 歷史紀錄排版優化：歷史紀錄行檔名可用寬度自適應、狀態文字寬度自適應靠右，消除大解析度螢幕或長檔名下的字元重疊與版面擠壓。
- 錯誤與取消精準判定：在使用者手動取消或發生轉檔錯誤時，不管是否已寫入部分檔案，歷史紀錄中皆能精確判定為「失敗」或「取消」，消除成功判定誤報。
- 翻譯設定介面重疊修正：移除 PDF 翻譯設定中重複多餘的「目標語言」標籤，完美消除設定分頁的文字重疊問題。
- 目標翻譯語言簡化：移除 PDF 翻譯選單中多餘且無效的目標語言選項，僅保留主要使用的「繁體中文」選項，精簡介面。

## 產品功能 (Product Features)
*(最多 20 個，以項目符號顯示)*
- 原生儀表板監測 Office 狀態
- 支援 Word/PPT 一鍵轉 PDF (需安裝 Office)

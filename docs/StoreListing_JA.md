...
Microsoft Partner Center にそのまま貼り付けられる、v3.6.0.0 向けのストア掲載情報です。
...
## What's new in this version
 - PDF 本地圧縮: 外部プログラムに依存せず、C#/GDI+ のネイティブエンジンで PDF を本地で圧縮する機能を実装。
 - 構造の最適化: ストリーム簡素化、フォント重複排除、フォント削除により PDF サイズを削減。
 - 高品質ダウンサンプリング: Bicubic による高品質な画像リサイズを実装し、小さい画像はぼやけないよう自動バイパス。
 - 設定スライダー UI: 設定タブに横型 4 段階スライダー（最小、小、標準、高品質）を追加し、UI 垂直スペースを大幅に節約。
...
## Short description
Clickra は、Office から PDF、PDF パスワード解除、PDF 結合、画像結合を高速に実行できるネイティブ右クリックメニュー ツールです。安全でローカル処理を重視します。

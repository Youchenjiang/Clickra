...
## 1. 視覺體驗與使用者介面 (Visuals & GUI) [F1]
為了提升 Clickra 的產品質感，後續將導入符合 Windows 11 Fluent Design 的視覺元素：

- [x] **[F1-1] 選單動態過濾 (Menu Visibility Optimization) [最高優先]**:
    - **精確顯示**: 已實作 `GetState` 邏輯，確保在不支援的檔案上完全隱藏選單。
- [x] **[F1-2] Native 儀表板實作 [v3.0.6]**:
    - 基於 Native Win32 與 AOT 實作，整合 Office 狀態偵測。
- [x] **[F1-3] 具備設定選項的 GUI (GUI with Options) [v3.0.8]**:
    - **轉換設定與歷史**: 提供 Dashboard 設定介面，顯示轉換歷史紀錄。
    - [x] **進度可視化 [v3.0.7]**: 處理過程中顯示動態進度條與完成通知。
- [x] **[F1-4] Windows 啟動體驗 (Startup Experience)**:
    - 已實作基本啟動顯示（暫不開發額外的現代化介面）。
- [x] **[F1-5] Dashboard 視窗與佈局修復 (GUI & Layout Fixes) [v3.0.9]**:
...
- [x] **[F1-6] Dashboard 細部功能優化 (Dashboard Minor Enhancements) [v3.0.9]**:
...
- [x] **[F1-7] 視窗行為與系統匣最佳化、歷史紀錄細緻化、右鍵選單優化 [v3.1.0]**:
...
- [x] **[F1-8] 儀表板歷史排版優化、選單目標語言簡化與錯誤判定邏輯修正 [v3.2.0]**:
...
- [x] **[F1-9] PDF 去除密碼與進度視窗內嵌輸入框優化 [v3.3.1]**:
    - **內嵌式密碼輸入**: 實作於進度視窗內繪製與管理 Win32 Edit 控制項，實現無閃爍且支援 Enter 鍵送出、Esc 鍵取消的內嵌密碼輸入。 (已完成)
    - **加密狀態預檢**: 自動判定 PDF 是否有加密，無加密檔案直接顯示提示不進行重複要求。 (已完成)



## 2. 核心功能擴張 (Advanced Features) [F2]
- [ ] **文件處理工具**:
    - [x] **[F2-1] Word 轉 PDF (Word to PDF)**: 已完成實作。
    - [x] **[F2-2] PDF 去除密碼 (Remove PDF Password) [v3.3.0]**:
        - 支援右鍵選單一鍵去除 PDF 密碼保護。 (已完成)
    - [x] **[F2-3] Excel 轉 PDF (Excel to PDF)**: 整合微軟 Excel COM 與 LibreOffice 雙引擎轉檔支援。 (已完成)
    - [x] **[F2-4] LibreOffice 離線 Office 轉檔引擎 (LibreOffice Offline Office Engine) [v3.5.0]**:
    ...
    - [ ] **[F2-8] PDF 轉圖片 (PDF to Image)**: 一鍵將 PDF 頁面匯出為高品質 JPG/PNG/TIFF，支援自訂 DPI 渲染率、色彩模式與透明背景處理。
    - [ ] **[F2-9] PDF 轉 PPTX (PDF to PPTX)**: 並存/整合三種不同定位之模式，供使用者自選或依 PDF 類型自動推薦：
    ...
    - [ ] **[F2-10] 文件去識別化與隱私防護 (Document De-identification & Redaction)**:
    ...
    - [ ] **[F2-11] 智能多圖與掃描件拼合 (Intelligent Scanner Stitching)**:
...
- [ ] **進階 PDF 學術與 AI 工具 (Advanced PDF Academic & AI Utilities)**:
    - [x] **[F2-12] PDF 學術論文翻譯 (PDF Math Translate) [v3.2.0]**: 純 C# 原生實現（相容 Native AOT）。自動識別與保護 LaTeX 公式，目前已實作免金鑰 Google 翻譯引擎，具備併發控制與速率限制。
    - [x] **[F2-13] PDF 佈局修復、字型解析 TTC 映射與高併發翻譯優化 [v3.2.1]**: 解決中文字型與數學字型 TTC 閃退，實作 table 頁面動態橫向合併門檻維持表格列對齊，並重構為 Google Mobile 批次翻譯。
    - [ ] **[F2-14] PDF 雙語對照翻譯模式 (Bilingual/Dual-language Translation)**:
        - 參考 `PDFMathTranslate` 的雙語對照實現，支援在輸出文件保留原文與譯文對照，提供更靈活的閱讀體驗；並擴充公式保護層（利用特殊字型如 Symbol / Cambria Math 自動辨識）。
    - [ ] **[F2-15] PDF 結構化數據與混合解析 (PDF to Markdown/JSON with Hybrid Parser)**:
...
- [ ] **圖片處理強化**:
    - [ ] **[F2-16] PNG/JPG 批量轉換**: 支援多種常用格式間的快速互轉。
    - [ ] **[F2-17] 高品質縮圖**: 支援批量調整圖片尺寸並保留細節。
- [ ] **批次檔名與資料夾工具**:
    - [ ] **[F2-18] 資料夾與檔案批次命名**: 支援自訂數字規則、提取建立日期、固定字串與自動編號。可以直接在資料夾右鍵選單對「整個資料夾及其內含檔案」進行操作。
    - [ ] **[F2-19] 批次檔案分類**: 支援依副檔名、日期區間或檔名關鍵字，將檔案自動分類並移入對應的資料夾。
    - [ ] **[F2-20] 批量建立空資料夾**: 支援依指定命名規則與結構要求，一次建立多個指定結構的空資料夾。
- [ ] **文字與編碼工具**:
    - [ ] **[F2-21] 文字編碼與簡繁轉換**: 支援 Big5/GBK/UTF-8 互轉與原生 `LCMapStringEx` 簡繁字元互轉。
- [ ] **右鍵選單功能擴充 (Context Menu Enhancements)**:
    - [ ] **[F2-22] 資料夾右鍵直接轉換**: 支援直接在資料夾右鍵選單操作，一鍵將資料夾底下的所有支援檔案進行轉換（如 Word 批次轉 PDF、圖片轉 PDF 等），無須進入資料夾手動選取。

## 3. 專案架構與規範 (Refactoring) [R1]
...
- [ ] **[R1-3] 降低 CLI 與 GUI 視窗事件的圈複雜度 (Complexity Reduction) [技術債]**:
...
- [ ] **[R1-5] 品質閘門誤報與基線觀察 (2026/08/08 記錄, Static Analysis Triage)**：
...
- [ ] **[R1-6] 開發期測試後門與倉庫整潔清理 (Dev Scaffolding Cleanup)**：
...
## 4. 維護、診斷與離線轉檔插件 (Diagnostics & Offline Fallback) [F3]
- [x] **[F3-1] 一鍵診斷回報與郵件反饋 (One-click Diagnostic Feedback) [v3.0.9]**:
    - 提供本地 Native AOT 診斷日誌記錄。於 Dashboard 實作一鍵郵件回報功能，自動打包診斷日誌與系統資訊，透過 Gmail 網頁開啟預設撰寫畫面（並支援標準 mailto: 協議連結作為系統預設郵件用戶端之備用方案）。
- [ ] **[F3-2] 多語系一鍵診斷回報信件草稿 (Multi-language Support for Diagnostic Feedback Mail)**:
    - 依據使用者目前的介面語言，自動撰寫對應語言的 Gmail/mailto 郵件草稿內容與主旨。
- [x] **[F3-3] 本地免 Office 離線轉檔引擎 (Local LibreOffice Fallback Engine) [v3.5.0]**:
...
- [ ] **[F3-4] LibreOffice 遠端 manifest 與宣傳頁整合 (Remote Manifest & Website Integration)**:
...
- [ ] **[F3-5] LibreOffice 安裝維護體驗強化 (LibreOffice Setup Maintenance UX)**:
    - 補強下載中斷續傳、代理/企業網路提示、安裝取消復原、以及 Windows Installer pending restart 場景的更細緻狀態說明。
- [ ] **[F3-6] Office 轉檔引擎抽象化與整合測試 (Office Engine Abstraction & Integration Tests)**:
...
## 5. PDF 無障礙化與自動標籤 (PDF Accessibility & Auto-Tagging) [F4]
- [ ] **[F4-1] PDF 無障礙結構稽核與 Well-Tagged PDF (WTPDF) 重建**:
...
里程碑依第 1–5 節訂定，各節標題後方即其里程碑編號（與 GitHub 里程碑一致）：

| 里程碑 | 說明 |
| --- | --- |
| [F1] 視覺體驗與使用者介面 | 拆為 9 個子項里程碑 [F1-1]–[F1-9]，掛於第 1 節各子項 |
| [F2] 核心功能擴張 | 拆為 22 個子項里程碑 [F2-1]–[F2-22]，掛於第 2 節各子項 |
| [R1] 專案架構與規範 | 拆為 6 個子項里程碑 [R1-1]–[R1-6]，掛於第 3 節各子項 |
| [F3] 維護、診斷與離線轉檔插件 | 拆為 6 個子項里程碑 [F3-1]–[F3-6]，掛於第 4 節各子項 |
| [F4] PDF 無障礙化與自動標籤 | 拆為 1 個子項里程碑 [F4-1]，掛於第 5 節 |

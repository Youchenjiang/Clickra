...
- [x] **v3.2.1.0 PDF 佈局修復、字型解析 TTC 映射與高併發翻譯優化** (2026/06/12)：解決中文字型與數學字型 TTC 閃退，實作 table 頁面動態橫向合併門檻維持表格列對齊，並重構為 Google Mobile API 支援 batch 翻譯、併發 semaphore 控制與隨機延遲等防 Ban 限流機制。

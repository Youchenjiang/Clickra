...
- **第五階段**：開發「進階 PDF 學術與 AI 工具」（引進 XY-Cut++ 閱讀順序分析重建多欄排版 Markdown，並支援本地與 AI 混合解析模式：本地快速解析搭配 Gemini 輔助表格與圖表描述提取；另擴充 PDF 雙語對照翻譯與公式特殊字型防護）。
- **第六階段**：開發「PDF 無障礙化與自動標籤」（支援對 PDF 結構進行無障礙稽核，並結合版面分析自動為未標記的 PDF 重建結構樹樹狀圖，生成符合 Well-Tagged PDF 標準之無障礙 Tagged PDF）。
- **第七階段**：開發「隱私去識別化與智能掃描拼合工具」：
    - 去識別化：實作純本地正則加校驗碼的 PII 自動識別、PDF 實體物理抹除 (Redaction) 及同格式/顏色/位置遮罩重寫 (Mask) 引擎，配合本地/雲端 AI 做語意漏抓檢索。

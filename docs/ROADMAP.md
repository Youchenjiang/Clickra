...
- [x] **專案目錄重整與模組化架構** (2026/05/15)：完成核心與 CLI 的解耦與 AOT 轉型。
- [x] **v3.0.7 動態進度條與完成通知** (2026/05/17)：完成純 Win32 進度視窗與原生 Toast 通知整合。
- [x] **選單動態過濾優化** (2026/05/19)：解決多檔案選單動態顯示與空選單過濾。

## 🚀 後續預計里程碑 (Upcoming Milestones)
- **第一階段**：開發「轉換設定與歷史」（於 Dashboard 介面整合轉換歷史紀錄與轉換設定功能）。

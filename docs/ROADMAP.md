...
- [x] **[F1-1] 選單動態過濾 (Menu Visibility Optimization)**:
    - **精確顯示**: 已實作 `GetState` 邏輯，確保在不支援的檔案上完全隱藏選單。
- [x] **[F1-2] Native 儀表板實作 (Native Dashboard Implementation) [v3.0.6]**:
...
- [x] **[F1-7] 視窗行為與系統匣最佳化 (Window Behavior & System Tray Optimization) [v3.1.0]**:
...
- [x] **[F1-8] 儀表板歷史排版優化 (Dashboard History Layout Optimization) [v3.2.0]**:
...
- [x] **[F1-9] 進度視窗內嵌輸入框優化 (Progress Window Inline Input Optimization) [v3.3.1]**:
...
- **進階 PDF 學術與 AI 工具 (Advanced PDF Academic & AI Utilities)**
    ...
    - [x] **[F2-13] PDF 佈局修復與高併發翻譯優化 (PDF Layout Fixes & High-concurrency Translation Optimization) [v3.2.1]**: 解決中文字型與數學字型 TTC 閃退，實作 table 頁面動態橫向合併門檻維持表格列對齊，並重構為 Google Mobile 批次翻譯。
...
- **圖片處理強化**
    - [ ] **[F2-16] PNG/JPG 批量轉換 (PNG/JPG Batch Conversion)**: 支援多種常用格式間的快速互轉。
    - [ ] **[F2-17] 高品質縮圖 (High-quality Thumbnails)**: 支援批量調整圖片尺寸並保留細節。
- **批次檔名與資料夾工具**
    - [ ] **[F2-18] 資料夾與檔案批次命名 (Folder & File Batch Renaming)**: 支援自訂數字規則、提取建立日期、固定字串與自動編號。可以直接在資料夾右鍵選單對「整個資料夾及其內含檔案」進行操作。
    - [ ] **[F2-19] 批次檔案分類 (Batch File Categorization)**: 支援依副檔名、日期區間或檔名關鍵字，將檔案自動分類並移入對應的資料夾。
    - [ ] **[F2-20] 批量建立空資料夾 (Batch Create Empty Folders)**: 支援依指定命名規則與結構要求，一次建立多個指定結構的空資料夾。
- **文字與編碼工具**
    - [ ] **[F2-21] 文字編碼與簡繁轉換 (Text Encoding & Traditional/Simplified Conversion)**: 支援 Big5/GBK/UTF-8 互轉與原生 `LCMapStringEx` 簡繁字元互轉。
- **右鍵選單功能擴充 (Context Menu Enhancements)**
    - [ ] **[F2-22] 資料夾右鍵直接轉換 (Folder Right-click Direct Conversion)**: 支援直接在資料夾右鍵選單操作，一鍵將資料夾底下的所有支援檔案進行轉換（如 Word 批次轉 PDF、圖片轉 PDF 等），無須進入資料夾手動選取。
...
- [x] **[R1-1] 模組化拆分 (Modularization)**:
    - 已完成 `src/Clickra.UI` 與 `src/Clickra.Core` 的解耦與 AOT 轉型。
- [ ] **[R1-2] 檔案命名整理 (File Naming Cleanup)**:
    - 統一整理專案內的檔案命名規範，消除歷史遺留的不一致命名。
- [ ] **[R1-3] 圈複雜度重構 (Complexity Reduction) [技術債]**:
...
- [ ] **[R1-4] 測試套件架構標準化 (Test Suite Architecture Standardization) [技術債]**:
...
- [ ] **[R1-5] 品質閘門觀察 (Quality Gate Observations)**：
...
- [ ] **[R1-6] 開發期清理 (Dev Scaffolding Cleanup)**：
...
- [ ] **[F4-1] PDF 無障礙稽核與 WTPDF 重建 (PDF Accessibility Audit & WTPDF Rebuild)**:
